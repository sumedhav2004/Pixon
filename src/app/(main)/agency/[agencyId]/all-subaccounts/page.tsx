  ...
  return (
    <AlertDialog>
      <div className='flex flex-col  h-full'>
        ...
        <Command className='rounded-lg bg-transparent h-full border'>
          <CommandInput placeholder='Search Account...' />
          <CommandList>
            <CommandEmpty>No results Found</CommandEmpty>
            <CommandGroup heading="Sub Accounts" className='h-fit border'>
              {!!user.Agency?.SubAccount.length
              ? user.Agency.SubAccount.map((subaccount: SubAccount) =>{
                console.log("Rendering subaccount: ",subaccount.name)
                return  (
                  <CommandItem key={subaccount.id} className='h-32 !bg-background my-2 border-[1px] border-border p-4 rounded-lg hover:!bg-background cursor-pointer transition-all'>
