...
const InfoBar = ({ notifications, subAccountId, className, role}: Props) => {
  ...
  return (
    <div className={twMerge(
      "fixed z-[20] md:left-[300px] left-0 right-0 top-0 p-4 bg-background/80 backdrop-blur-md flex gap-4 items-center border-b-[1px]", className
      )}>
            ...
            <SheetContent className='mt-4 mr-4 pr-4 overflow-scroll'>
              <SheetHeader className='text-left'>
                ...
                <SheetDescription>{(role === 'AGENCY_ADMIN' || role==='AGENCY_OWNER') && (
                  <Card className='flex items-center justify-between p-4 border-none mb-8'>
                    Current SubAccount
              ...
              {allNotifications?.map((notification) => (
                <div key={notification.id} className='flex flex-col gap-y-2 mb-4 text-ellipsis'>
